...
export default function InternalComms({ 
  ...
  // صلاحية استعراض الـ IPs والتحكم بالشبكة
  const canViewIPs = currentUserRole === 'admin';
  ...
  const [blockRecipient, setBlockRecipient] = useState(false);
  ...
  const userInbox = messages.filter(msg => {
    ...
    // ضمان وصول الرسائل المتبادلة دون حجب
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-150 shadow-xs space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-50 pb-3">
            ...
            >
              ...
              <span>{showNetworkSettings ? 'إغلاق لوحة الـ IP' : 'تخصيص الـ IP لكل جهاز بالأقسام'}</span>
            </button>
          </div>
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      {onDeleteMessage && (isMine || currentUserRole === 'admin') && (
                        <button 
                          onClick={() => onDeleteMessage(msg.id)}
                          className="bg-red-50 hover:bg-rose-600 hover:text-white border border-rose-200 text-rose-700 p-0.5 px-1.5 rounded-md font-bold text-[9px] flex items-center justify-center cursor-pointer active:scale-95 transition-all"
                          title="مسح وحذف الخطاب من الصندوق كلياً"
                        >
        ...
        <div className="lg:col-span-12 xl:col-span-5 bg-white p-5 rounded-2xl border border-slate-150 shadow-xs space-y-4">
          ...
          <form onSubmit={handleComposeSubmit} className="space-y-4">
            ...
            <div className="space-y-1 text-xs">
              ...
              {/* خيار الحجب المتبادل الصامت الفعال كوضع افتراضي لإرسال الرسائل */}
              <div className="mt-2.5 p-3 bg-red-50/40 hover:bg-red-50/70 border border-red-100/80 rounded-xl space-y-1.5 transition-all">
                <label className="flex items-start gap-2.5 cursor-pointer select-none">
                  <input 
                    type="checkbox" 
                    checked={blockRecipient}
                    onChange={(e) => setBlockRecipient(e.target.checked)}
                    className="mt-0.5 accent-red-650 rounded cursor-pointer w-4 h-4 shrink-0"
                  />
                  <div className="text-right">
                    <span className="font-bold text-[11px] text-red-900 block leading-normal">
                      🔕 كتم تسليم الصادر للمستقبل (المستقبل لا يصله بريدي، وأنا أستلم منه)
                    </span>
                    <span className="text-[10px] text-slate-500 block mt-0.5 leading-relaxed font-normal font-sans">
                      عند تحديد هذا الخيار، لن يستلم الطرف الآخر هذه المراسلة في لوحته، في حين تظهر لديك بصندوق الصادر وتستمر باستلام خطاباته بشكل آمن ومتبادل.
                    </span>
                  </div>
                </label>
              </div>
