  ...
  const getDocExpiryStats = (doc: RequiredDocument) => {
    ...
    if (diffDays < 0) {
      return { 
        ...
        status: 'expired', 
        color: 'text-red-900 bg-red-50/80 border border-red-200',
        critical: true
      };
    } else if (diffDays <= 30) {
      return { 
        ...
        status: 'warning', 
        color: 'text-yellow-900 bg-yellow-50/80 border border-yellow-200',
        critical: true
      };
    }
    return { 
                ...
                return (
                  ...
                  >
                    ...
                    {doc.provided && doc.expiryDate && (
                      <div className={`p-2.5 rounded-lg text-[11px] ${expiryStats.color} flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mt-1.5 shadow-2xs`}>
                        <div className="flex items-center gap-1.5">
                          {expiryStats.status === 'expired' ? (
                            <span className="flex items-center justify-center p-1 rounded bg-red-100 text-red-600 animate-pulse">
                              <AlertTriangle className="w-3.5 h-3.5 shrink-0" />
                            </span>
                          ) : expiryStats.status === 'warning' ? (
                            <span className="flex items-center justify-center p-1 rounded bg-yellow-100 text-yellow-700 animate-pulse">
                              <AlertTriangle className="w-3.5 h-3.5 shrink-0" />
                            </span>
                          ) : (
                            <Clock className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                          )}
                          <span className="font-bold leading-normal">{expiryStats.label}</span>
                        </div>
                        
                        <div className="flex items-center gap-1">
                          {expiryStats.status === 'expired' && (
                            <span className="flex items-center gap-1 bg-red-600 text-white px-2 py-1 rounded-md font-black font-mono text-[10px] shadow-sm border border-red-700 animate-pulse">
                              <AlertTriangle className="w-3 h-3 text-white shrink-0" />
                              <span>{doc.expiryDate}</span>
                            </span>
                          )}
                          {expiryStats.status === 'warning' && (
                            <span className="flex items-center gap-1 bg-yellow-400 text-yellow-950 px-2 py-1 rounded-md font-black font-mono text-[10px] shadow-sm border border-yellow-500">
                              <AlertTriangle className="w-3 h-3 text-yellow-950 shrink-0" />
                              <span>{doc.expiryDate}</span>
                            </span>
                          )}
                          {expiryStats.status === 'valid' && (
                            <span className="font-bold font-mono text-emerald-800 bg-emerald-100/80 px-2 py-0.5 rounded-md border border-emerald-250 text-[10px]">
                              {doc.expiryDate}
                            </span>
                          )}
                        </div>
          ...
          <div className="bg-white max-w-xl w-full rounded-2xl overflow-hidden shadow-2xl border border-slate-200 animate-scale-in pdf-style-container p-6 space-y-5 text-right printable-receipt">
            ...
            <div className="border-b-2 border-dashed border-slate-200 pb-4 text-center space-y-1">
              <div className="text-slate-800 font-extrabold text-lg flex items-center justify-center gap-2">
                ...
                <span>{localStorage.getItem('SYSTEM_TITLE') || 'النظام الالكتروني للجامعة'} - شعبة الإيرادات والحسابات</span>
              </div>
