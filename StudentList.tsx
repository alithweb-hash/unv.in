...
export default function StudentList({ 
  ...
  // صلاحية الإدارة للأكسيل حصرية لمدير التسجيل ومدير النظام وللعمداء (admin & registration_director & head_*)
  const isAuthorized = currentRole === 'admin' || currentRole === 'registration_director' || currentRole?.startsWith('head_');
  ...
  const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // استخدام ArrayBuffer وقراءته كـ Uint8Array يضمن فك ترميز الأحرف العربية والخاصة بشكل صحيح 100% دون تلف أو تشويه
        ...
        json.forEach((row, index) => {
          const rowNum = index + 2; // For user-friendly error messages indicating row index in excel path
          ...
          // تنظيف وتصفية الاسم كلياً من أي بادئات توضيحية أو حروف مكتوبة بجانبه (مثل: الاسم:، اسم الطالب/، الطالب، إلخ) للحفاظ على نقاء حقل الاسم الصافي فقط
          if (name) {
            let nameStr = String(name).trim();
            // إزالة الكلمات الإرشادية المتبوعة بنقطتين أو خط مائل أو مسافات
            nameStr = nameStr.replace(/^(الاسم|الاسم الكامل|اسم الطالب|الاسم الرباعي واللقب|الاسم الرباعي|الاسم الثلاثي|اسم|الاســم|أسم|الأسم|طالب|الطالب)\s*[:\-\/\\=\s]+/i, '');
            // إزالة كشف البادئات البسيطة المتبوعة بمسافة
            nameStr = nameStr.replace(/^(الاسم|الأسم|أسم|الطالب|طالب)\s+/i, '');
            // إزالة أي علامات ترقيم زائدة في البداية والنهاية
            nameStr = nameStr.replace(/^[:\-\/\\=\s]+|[:\-\/\\=\s]+$/g, '').trim();
            name = nameStr;
          }

          // تخطي الصفوف التي تحتوي على مسميات تصف طبيعة الحقل كـ (اسم الطالب، الطالب، الاسم، إلخ) والتي تظهر كعناوين أو إرشادات ترويسية في ملف الإكسيل
          if (name) {
            const cleanName = String(name).trim();
            if (cleanName === 'الطالب' || cleanName === 'اسم الطالب' || cleanName === 'الاسم الكامل' || cleanName === 'الاسم' || cleanName === 'اسم' || cleanName === 'الاسم الثلاثي' || cleanName === 'الاسم الرباعي واللقب' || cleanName === 'الاســم' || cleanName === 'أسم' || cleanName === 'الأسم') {
              skippedCount++;
              return; // تخطي ترويسة ملف الاكسيل الزائدة كلياً ليكون الجدول مطابقاً تماماً
            }
          }

          const residence = getRowValByPatterns(row, ['السكن', 'سكن', 'محافظة السكن', 'المحافظة', 'العنوان', 'محافظه', 'residence', 'address']) || 'بابل';
          let phone = getRowValByPatterns(row, ['تلفون', 'الهاتف', 'هاتف', 'رقم الموبايل', 'رقم الهاتف', 'موبايل', 'جوال', 'تليفون', 'phone', 'mobile']);
          let deptName = getRowValByPatterns(row, ['القسم', 'قسم', 'القسم العلمي', 'التخصص', 'تخصص', 'department', 'dept', 'كلية', 'الكلية']);
          ...
          // التحقق الذكي من وجود البيانات الإلزامية وتجنب الرفض الصارم في حال توفر الاسم
          const missingFields: string[] = [];
          if (!name || !String(name).trim()) {
            missingFields.push('اسم الطالب');
          }
          
          // في حال عدم توفر القسم أو رقم الهاتف، بدلاً من رفض الصف كلياً وتفشيل الاستيراد، نقوم بوضع قيم افتراضية ذكية لتسهيل عمل مدخل البيانات مع إشعاره بذلك
          if (!deptName || !String(deptName).trim()) {
            deptName = departments[0]?.name || 'هندسة تقنيات الحاسوب';
          }
          if (!phone || !String(phone).trim()) {
            phone = '07700000000'; // رقم افتراضي مؤقت للاتصال
          }

          if (missingFields.length > 0) {
            skippedCount++;
            const studentLabel = name ? `الطالب "${name}"` : `الصف رقم (#${rowNum})`;
            validationErrors.push(`• ${studentLabel}: الحقل المفقود (${missingFields.join(' و ')})`);
            return; // تخطي معالجة هذا الصف لغياب البيانات الإلزامية العتبية
          }

          const resolvedDept = resolveDeptId(String(deptName));
          ...
          const importedStudent: Student = {
            ...
            notes: 'تم الاستيراد والتدقيق التلقائي عبر ملف Excel الخماسي المعتمد ومطابقة النواقص العينية',
            sentToCollege: true,
        ...
        if (validationErrors.length > 0) {
          const errMsg = `⚡ تم استيراد وتحميل (${importCount}) طالب بنجاح.\n\n⚠️ تنبيه التدقيق: تم تخطي وتجاوز (${skippedCount}) صفوف بسبب عدم صلاحية أو فقدان الحقول الإلزامية:\n${validationErrors.slice(0, 10).join('\n')}${validationErrors.length > 10 ? `\n...وغيرها (${validationErrors.length - 10} حقول مفقودة من سطور مختلفة)` : ''}`;
          alert(errMsg);
        } else {
          alert(`⚡ تم الاستيراد بنجاح! تم قراءة وتحليل كافة الصفوف وتسجيل (${importCount}) طالب بنجاح بعد التحقق والمطابقة التامة.`);
        }
          ...
          <div className="mx-5 my-4 p-4.5 bg-indigo-50/50 border border-indigo-150 rounded-xl space-y-3.5 animate-fade-in text-right">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="text-[10px] bg-indigo-100 text-indigo-800 px-2.5 py-1 rounded-md font-bold uppercase shrink-0">
                الصلاحية الحالية المعتمدة: {currentRole === 'admin' ? 'مدير النظام 🛡️' : currentRole?.startsWith('head_') ? 'عميد الكلية 🏢' : 'مدير التسجيل الفعال 🎓'}
              </div>
            </div>
          ...
          <table className="w-full text-right border-collapse text-xs md:text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-700 font-extrabold whitespace-nowrap">
                ...
                <th className="p-3 text-center border-b border-slate-200">القسم العلمي / الكلية</th>
                <th className="p-3 text-center border-b border-slate-200">رقم الهاتف</th>
              ...
              {filteredStudents.map((student, index) => {
                ...
                return (
                  <tr key={student.id} className="hover:bg-slate-50/70 transition-colors whitespace-nowrap">
                    ...
                    {/* القسم العلمي / الكلية */}
                    <td className="p-3 text-center border-b border-slate-100 font-bold text-slate-700">
                      {dept?.name || student.departmentId}
                    </td>

                    {/* رقم الهاتف */}
                    <td className="p-3 text-center border-b border-slate-100 font-mono text-slate-650 font-bold">
                      {student.phone}
                    </td>
                    ...
                    <td className="p-3 border-b border-slate-100">
                      <div className="flex items-center gap-1.5 justify-end">
                        ...
                        {(currentRole === 'admin' || currentRole === 'registration_director' || currentRole?.startsWith('head_')) && (
                            ...
                            {deletingStudentId === student.id ? (
                              <div className="flex items-center gap-1">
                                <button 
                                  ...
                                  className="bg-red-650 hover:bg-red-750 text-white font-extrabold text-[10px] px-2 py-1 rounded-md flex items-center gap-1 cursor-pointer animate-pulse transition-all shadow-sm"
                                  title="اضغط لتأكيد الشطب والمسح بشكل نهائي"
                                >
                                ...
                                <button 
                                  ...
                                  className="bg-slate-200 hover:bg-slate-300 text-slate-800 font-extrabold text-[10px] px-1.5 py-1 rounded-md flex items-center justify-center cursor-pointer transition-all"
                                  title="إلغاء وتراجع عن الحذف"
                                >
                                  <span>تراجع</span>
                                </button>
                              </div>
                              ...
                              <button 
                                ...
                                onClick={() => {
                                  ...
                                  setTimeout(() => {
                                    setDeletingStudentId(prev => prev === student.id ? null : prev);
                                  }, 5000);
                                }}
                                className="bg-red-55/10 hover:bg-red-600 hover:text-white text-red-650 border border-red-200 transition-all font-bold text-[10px] px-2 py-1 rounded-md flex items-center gap-1 cursor-pointer"
